...
export function ProfileCard({ resources }: { resources: ProfileResources }) {
  ...
  return (
    <>
      ...
      <article className="profile-reveal overflow-hidden rounded-2xl border border-(--demo-line) bg-(--demo-surface) shadow-[0_1px_0_rgba(15,23,42,0.04)]">
        <div className="flex items-center gap-4 px-5 py-5">
          ...
          <div className="min-w-0">
            <h2 className="profile-name truncate text-[1.65rem] leading-none font-semibold tracking-tight text-(--demo-ink)">
              {profile.name}
            </h2>
            <p className="profile-bio mt-1.5 truncate text-[1.05rem] leading-snug text-(--demo-muted)">
              {profile.description}
            </p>
          </div>
        </div>
